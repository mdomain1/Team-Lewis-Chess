...
public class Pawn extends Piece {
    ...
    static public boolean isValidMove(int[][] fPieceTypeLocationsOnBoard, Team fTeam)
    {
        if(Pawn.withinRangeOfPieceMobility(fPieceTypeLocationsOnBoard,fTeam)){
            if(Pawn.noPieceBlocksPathToSquare(fPieceTypeLocationsOnBoard,fTeam)){
                if(Pawn.moveDoesNotPlaceKingInCheck(fPieceTypeLocationsOnBoard, fTeam)){
                    return true;
                }
            }
        }       
    ...
    static private boolean withinRangeOfPieceMobility(int[][] fPieceTypeLocationsOnBoard, Team fTeam)
    {
        ...
        if (Game.getCurrentTeamsTurn() == 0) { //white's turn
            if (columnClicked == columnTargeted) { //moving forward either one or two
                if (rowTargeted == rowClicked + 1 || (rowTargeted == rowClicked + 2 && rowClicked == 4)) {
                    return true; //WithinRangeOfPieceMobility
                }
            } else if ((columnClicked == columnTargeted + 1 || columnClicked == columnTargeted - 1) && rowTargeted == rowClicked + 1) {
            //capturing normally or capturing en passant
                if (fPieceTypeLocationsOnBoard[rowClicked][columnClicked] >= 7 && fPieceTypeLocationsOnBoard[rowClicked][columnClicked] <= 12) {
                    return true; //WithinRangeOfPieceMobility
                } else if (fTeam.pawnMovedTwoSpacesLastMove && (Board.getColumnFromLocation(fTeam.squarePawnMovedTwoLastMovedTo) == columnTargeted + 1
                || Board.getColumnFromLocation(fTeam.squarePawnMovedTwoLastMovedTo) == columnTargeted - 1) && rowClicked == 2) {
                    return true; //WithinRangeOfPieceMobility
                }
            }
        } else { //black's turn
            if (columnClicked == columnTargeted) { //moving forward either one or two
                if (rowTargeted == rowClicked - 1 || (rowTargeted == rowClicked - 2 && rowClicked == 3)) {
                    return true; //WithinRangeOfPieceMobility
                }
            } else if ((columnClicked == columnTargeted + 1 || columnClicked == columnTargeted - 1) && rowTargeted == rowClicked - 1) {
            //capturing normally or capturing en passant
                ...
                || Board.getColumnFromLocation(fTeam.squarePawnMovedTwoLastMovedTo) == columnTargeted - 1) && rowClicked == 5) {
                    return true; //WithinRangeOfPieceMobility
                }
            }
        }
        
        return false; //Not withinRangeOfPieceMobility
    }
    ...
    static private boolean noPieceBlocksPathToSquare(int[][] fPieceTypeLocationsOnBoard, Team fTeam)
    {
        ...
        if (Game.getCurrentTeamsTurn() == 0) { //white's turn
            if (columnClicked == columnTargeted) { //moving forward either one or two
                if (rowTargeted == rowClicked + 1 && fPieceTypeLocationsOnBoard[rowClicked][columnClicked] == 0) {
                    return true; //noPieceBlocksPathToSquare
                } else if (rowTargeted == rowClicked + 2 && rowClicked == 4 && fPieceTypeLocationsOnBoard[rowClicked + 1][columnClicked] == 0
                && fPieceTypeLocationsOnBoard[rowClicked][columnClicked] == 0) {
                    return true; //noPieceBlocksPathToSquare
                }
            } else if ((columnClicked == columnTargeted + 1 || columnClicked == columnTargeted - 1) && rowTargeted == rowClicked + 1) {
            //capturing normally or capturing en passant
                ...
                || Board.getColumnFromLocation(fTeam.squarePawnMovedTwoLastMovedTo) == columnTargeted - 1)
                && Board.getColumnFromLocation(fTeam.squarePawnMovedTwoLastMovedTo) == columnClicked && rowClicked == 2) {
                    return true; //noPieceBlocksPathToSquare
                }
            }
        } else { //black's turn
            if (columnClicked == columnTargeted) { //moving forward either one or two
                if (rowTargeted == rowClicked - 1 && fPieceTypeLocationsOnBoard[rowClicked][columnClicked] == 0) {
                    return true; //noPieceBlocksPathToSquare
                } else if (rowTargeted == rowClicked - 2 && rowClicked == 3 && fPieceTypeLocationsOnBoard[rowClicked - 1][columnClicked] == 0
                && fPieceTypeLocationsOnBoard[rowClicked][columnClicked] == 0) {
                    return true; //noPieceBlocksPathToSquare
                }
            } else if ((columnClicked == columnTargeted + 1 || columnClicked == columnTargeted - 1) && rowTargeted == rowClicked - 1) {
            //capturing normally or capturing en passant
                ...
                || Board.getColumnFromLocation(fTeam.squarePawnMovedTwoLastMovedTo) == columnTargeted - 1)
                && Board.getColumnFromLocation(fTeam.squarePawnMovedTwoLastMovedTo) == columnClicked && rowClicked == 5) {
                    return true; //noPieceBlocksPathToSquare
                }
            }
        }
        
        return false; //There is a piece that blocks path to square.
    }
    
    static private boolean moveDoesNotPlaceKingInCheck(int[][] fPieceTypeLocationsOnBoard, Team fTeam)
    {
        int rowClicked = 0;
        int columnClicked = 0;
        int rowTargeted = 0;
        ...
        if (Game.getCurrentTeamsTurn() == 0) { //white's turn
            if ((columnClicked == columnTargeted + 1 || columnClicked == columnTargeted - 1) && rowTargeted == rowClicked + 1) {
            //capturing en passant
                if (fTeam.pawnMovedTwoSpacesLastMove && (Board.getColumnFromLocation(fTeam.squarePawnMovedTwoLastMovedTo) == columnTargeted + 1
                || Board.getColumnFromLocation(fTeam.squarePawnMovedTwoLastMovedTo) == columnTargeted - 1)
                && Board.getColumnFromLocation(fTeam.squarePawnMovedTwoLastMovedTo) == columnClicked && rowClicked == 2) {
                    tempArray[rowTargeted][columnClicked] = 0;
                }
            }
        } else { //black's turn
            if ((columnClicked == columnTargeted + 1 || columnClicked == columnTargeted - 1) && rowTargeted == rowClicked - 1) {
